...
/**
 * Replace parentheses which wrap the alias of 's-for' directive values by array brackets in order to avoid syntax errors.
...
/**
 * Normalize the `ForXStatement#left` node to parse s-for expressions.
...
function parseVOnExpressionBody(
    ...
    debug('[script] parse s-on expression: "void function($event){%s}"', code)

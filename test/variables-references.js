...
describe("[variables] elements", () => {
    describe("which have s-for directive", () => {
        const code = '<template><div s-for="a in b"></div></template>'
        let ast = null
...
describe("Variables of s-for and references", () => {
    const code =
        '<template><div s-for="x of xs" :key="x">{{x + y}}<div>{{x}}</div></div>{{x}}</template>'
    let variables = null
...
describe("Variables of s-for and references of dynamic arguments", () => {
    const code = '<template><div s-for="x of xs" :[x]="1" /></template>'
    let variables = null
    let vForReferences = null
    let vBindKeyReferences = null
